...
| 发布 tag | ✅ 已创建并推送 | `v1.1-b1` 指向 `main` 的最终发布记录提交 |
...
- [x] 在最终发布记录提交创建 annotated tag `v1.1-b1`。
- [x] 已推送 `main` 与 `v1.1-b1`，并确认 GitHub 上 tag 指向正确提交。
- [x] 已写入发布记录：发布负责人于 2026-08-10 确认 TestFlight 可安装和真机验证完成；未提供精确时间、设备型号与 iOS 版本。
- [ ] 若发版后发现阻塞问题，优先停止测试分发并递增 build 修复，不复用或移动已发布 tag。

## 8. 发布负责人结论

`v1.1-b1` 已上线、验证完成并完成 Git 收口。若后续发现阻塞问题，保持已发布 tag 不变，递增 build 后重新走完整发布流程。
