...
- [x] 使用旧 TestFlight build 20 做最小兼容回归：Apple 登录、训练拉取/同步、Team 列表和动态正常；用户确认通过。
- [x] 记录后端部署时间、备份位置、部署 commit 和迁移结果。

## 3. iOS TestFlight 上传

- [x] 打开 `ios/DontLift/DontLift.xcodeproj`，Scheme 选择 `DontLift`。
- [x] 目标设备选择 `Any iOS Device (arm64)`。
- [x] 确认 General 中版本为 `1.0`、build 为 `21`，主 App 与 widget extension signing 正常。
- [x] 确认 App Group `group.com.yulinxi.app.DontLift` 和 Push Notifications entitlement 可用于 App Store 分发签名。
- [x] 执行 `Product -> Archive`。
- [x] 在 Organizer 中选择本次 `1.0 (21)` archive。
- [x] 执行 `Distribute App -> App Store Connect -> Upload`。
- [x] 等待 App Store Connect 处理完成，状态为 `VALID`；用户于 2026-07-16 22:28 CST 确认。
- [x] 在 TestFlight 中确认出现 `1.0 (21)` 并完成真机回归。
- [x] 记录 App Store Connect build 状态；精确 Archive、上传和处理完成时间未提供。

## 4. TestFlight 主流程回归

### 4.1 基础回归

- [x] Apple 登录、冷启动、退出重登和正式同步路径正常。
- [x] 训练首页、计划页、动作库、Team 页和我的页面加载正常，无 401/403、持续 Loading 或空白卡死。
- [x] Live Activity、组间休息通知、HealthKit 写入和训练摘要小组件没有回归。

### 4.2 Team 拍一拍与消息设置

- [x] 账号 A、B 加入同一 Team，尚无今日动态的队友出现在可拍列表，当前用户本人不出现。
- [x] 账号 A 拍账号 B 后立即显示“已拍”，重复进入页面仍保持；同日不能重复发送第二条。
- [x] 账号 B 收到包含发送者与 Team 名称的通知，点击后进入对应 Team。
- [x] 已有今日 Team 动态或已关闭当前 Team 消息的成员不可拍。
- [x] 关闭“Team消息”后，当前 Team 的拍一拍、队友打卡和表情回应通知均停止；其他 Team 不受影响。
- [x] “分享动态”和“Team消息”分别切换、快速返回、重进页面后状态正确，保存过程中设置卡不抖动。
- [x] 关闭“分享动态”后完成训练不产生新 Team 动态；重新开启流程与隐私确认正常。

### 4.3 计划与备选动作

- [x] 普通计划动作可添加、排序和移除备选；递减组和超级组不出现备选入口。
- [x] 个人计划、Team 分享版本、Fork 和离线重启后备选动作仍存在，Team 分享不泄露重量。
- [x] 从计划开始训练后，在没有完成任何组时可切换备选；完成热身或正式组后不可切换。
- [x] 严格模式切换备选后保留组次结构并清空重量；切回默认动作恢复默认落值。
- [x] 自适应模式只使用该计划动作位下该备选自身的历史，不借用默认动作重量。
- [x] 完成备选动作后实际历史和 PR 正常，默认计划动作与安排不被覆盖。
- [x] 计划详情默认折叠，最多展开一个动作；普通、递减组、超级组的组次详情和下次重量正确。
- [x] 长计划可连续滚动和左边缘侧滑返回；右侧 `…` 菜单可编辑、删除并保留二次确认。

### 4.4 分享海报

- [x] 普通训练、PR、Team 来源和复杂训练分别得到合理的初始背景推荐。
- [x] 左右滑动海报、左右按钮和小预览均能在 5 款背景间切换，页码与选中态同步。
- [x] 海报至少展示 6 个动作，超过 6 个时显示剩余数量，文字不遮挡主要人物或器械。
- [x] 当前海报保存到相册和系统分享后的图片均为 `1080×1920`，与屏幕预览背景和数据一致。
- [x] 离线状态下仍可切换全部背景、保存和分享。

### 4.5 可靠性回归

- [x] 正式组标记为热身再取消后恢复原相对顺序；旧热身数据一次操作即可取消。
- [x] 最后一组休息尚未结束时直接结束训练，休息记录按本段目标时长保存，计时器和 Live Activity 正常停止。
- [x] 历史日期训练跨日补同步后仍出现在原日期，队友不会收到“今天完成训练”通知。
- [x] 训练摘要小组件跨日和跨周后使用当前日期与周范围，不残留旧日高亮。
- [x] 设置页休息时长、估算体重和法律页 sheet 高度稳定，保存、取消和键盘交互正常。

## 5. 合并、Tag 与推送

> 生产后端、TestFlight `VALID` 和真机回归门禁均已通过，可以进入 `main` 合并与 tag 阶段。
...
- TestFlight 上传时间：2026-07-16，精确时间未提供
