...
xcodebuild -project MeiGei.xcodeproj -scheme MeiGei \
  -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17 Pro' \
  -configuration Debug CODE_SIGNING_ALLOWED=NO build
```

- 工程用 Xcode 26 的 `PBXFileSystemSynchronizedRootGroup`：往 `ios/MeiGei/MeiGei/` 目录新增 `.swift` 文件即自动纳入 target，**无需手改 pbxproj**（仅 build settings/entitlements/新 target 才需改 pbxproj）。
...
- **Live Activity**：独立 target `MeiGeiWidgetsExtension`（传统 target，Sources phase 显式列文件）；`RestActivityAttributes`/`EndRestIntent` 同时编进 app 与 widget 两端（ActivityKit 按类型名跨进程匹配）。倒计时靠 `Text(timerInterval:)` 自走，无需推送；「提前结束」经 App Intent → Darwin 通知回传主 App。
