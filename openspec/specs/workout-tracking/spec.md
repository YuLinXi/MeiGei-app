...
计划列表的模式行为摘要 SHALL 保持现有行为：每个计划卡 SHALL 展示当前模式，并 SHALL 用一行短文案说明行为差异（如“下次依据：上次完成实绩”“完成后自动更新”“严格执行 · 不回写”“默认 4×10 起步”）。用户 MUST NOT 只有进入详情页后才知道计划是否会自动回写。
...
#### Scenario: 保存计划后同步训练名称
- **WHEN** 用户将一次已完成的可保存训练保存为计划模板
- **THEN** 系统将本次训练名称更新为用户确认的计划名称
- **AND** 该训练进入既有同步队列
- **AND** 系统不追溯修改已经生成的 Team 打卡摘要
...
### Requirement: 训练交互与历史准备解耦

当前训练解析完成后，客户端 SHALL 立即展示可操作的训练页，不等待全部历史。历史的持久化读取、关系遍历、排序和值快照聚合 MUST 在后台执行，只允许 Sendable 值跨执行器传递。动作别名 SHALL 按不同标识批量解析。主线程只维护加载状态与发布结果。

#### Scenario: 首次进入与历史刷新
- **WHEN** 当前训练已解析，而历史快照尚未就绪
- **THEN** 用户可以编辑、勾选、滚动、收起与恢复训练
- **AND** 只有历史相关区域显示加载状态，未加载 MUST NOT 当作没有历史
- **AND** 刷新期间已有快照保留；失败显示重试且不清除旧快照

#### Scenario: 历史预填等待
- **WHEN** 用户启动依赖历史预填的计划或新增动作流程
- **THEN** 只等待该流程的历史依赖，并复用同一轮读取
- **AND** 不在用户编辑后异步覆盖重量、次数、组结构或动作选择

#### Scenario: 刷新期间修改或退出登录
- **WHEN** 后台读取期间历史再次变更或当前账号退出
- **THEN** 过期结果不得发布到页面
- **AND** 同账号变更合并补跑；退出登录清除快照并隔离旧任务

### Requirement: 训练保存与提醒关键路径

进行中训练的勾选、输入、休息回写 SHALL 立即修改本地字段和待同步标记，保留 300ms 保存防抖及离开、切后台立即保存。能够由保存通知标识明确归属到进行中训练的变更 MUST NOT 使已完成徽章历史失效；完成、编辑、删除历史、导入、同步及计划或徽章变化 SHALL 正确刷新。未知归属或删除 SHALL 保守刷新，分类不得扫描全部历史。

#### Scenario: 连续勾选与输入
- **WHEN** 用户连续勾选、取消或输入并触发保存
- **THEN** 已完成徽章历史读取次数不增加
- **AND** 同一次完成操作只进行一次页面统计重算
- **AND** 几何值相同时不重复写回，组和动作身份保持稳定

#### Scenario: 音频准备及取消
- **WHEN** 用户开始休息
- **THEN** 提示音准备在专用串行执行单元异步执行，点击处理不等待播放器或音频会话初始化
- **AND** 播放、停止、会话压低及恢复按顺序执行，准备不提前激活会话
- **WHEN** 用户关闭声音、结束训练或退出登录
- **THEN** 排队播放请求失效，保持后台本地通知和前后台避免双响的既有语义

#### Scenario: 休息浮层与 Live Activity
- **WHEN** 用户拖动浮层、编辑键盘或倒计时变化
- **THEN** 状态在实际消费它的子视图内观察，行位置只在聚焦时测量
- **AND** Live Activity 跳过相同快照并合并操作中间状态，保留休息起止、训练结束与切后台更新，不引入固定延迟
