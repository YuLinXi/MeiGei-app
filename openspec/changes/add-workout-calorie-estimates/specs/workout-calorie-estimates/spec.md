...
iOS SHALL 基于已完成训练的时长、用户本地设置的估算体重和粗粒度强度系数计算 kcal 估算值。估算结果 MUST 由本地原始训练记录即时派生或在本地可重建快照中派生，MUST NOT 作为 `Workout` 同步真相字段上传。展示文案 MUST 使用「约」表达估算性质。

#### Scenario: 常规力量训练估算
- **WHEN** 用户已设置估算体重且查看一条有有效时长的已完成训练
- **THEN** iOS 按 `MET × 3.5 × bodyWeightKg / 200 × durationMinutes` 计算整数 kcal
- **AND** 展示文案包含「约」与 `kcal`
...
训练分享海报 SHALL 在 kcal 估算可用时重点展示 `约 xxx kcal`，并保持其它训练指标可读。用户关闭消耗估算或未设置估算体重时，海报 MUST 不展示 kcal。

#### Scenario: 海报展示 kcal
- **WHEN** 用户预览一张 kcal 估算可用的训练分享海报
- **THEN** 海报指标区展示 `约 xxx kcal`
- **AND** kcal 的视觉层级不低于时长、组数、动作数等摘要指标
