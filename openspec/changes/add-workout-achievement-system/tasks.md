...
- [x] 6.1 会话共享展示缓存、后台训练读取与进度计算、保存事件失效、体重更新与账号隔离；完成重复进入、同数量编辑、删除、并发刷新及 1,000 场数据回归测试。
- [x] 6.2 移除固定等待和整页加载提示，统一懒加载网格，保留实时图标绘制和中文展示。
- [x] 6.3 验证真实导航与连续滚动，记录性能验证结果和真机验收边界（见 `docs/badge-wall-performance-verification.md`）。
