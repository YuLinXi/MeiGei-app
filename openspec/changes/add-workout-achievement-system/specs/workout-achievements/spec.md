...
### Requirement: 辅助动作成就判定特例

系统 SHALL 遵循 `assisted-exercise-weight` 的固定辅助重量规则：辅助公斤不计吨位，真实辅助突破可计单场 PR 数量，辅助增加不得视为 PR。成就未上线阶段 SHALL 按版本一次性重建旧算法授予，重建完成后恢复永久授予；不得删除或改写训练原始记录。

#### Scenario: 修正未发布阶段错误吨位授予
- **WHEN** 旧授予只因辅助公斤错误累加而满足吨位门槛
- **THEN** 新规则的一次性重建移除该错误授予，后续按真实负重吨位判定
