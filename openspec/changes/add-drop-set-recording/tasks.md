...
- [x] 3.2 实现快速新增递减组：默认两个内部组，首个内部组从上一正式组预填，新增后不自动弹出键盘
- [x] 3.3 更新组级更多操作菜单，支持普通组改递减组、递减组改回普通组，并处理丢弃内部组确认
- [x] 3.4 实现递减组展开行 UI，展示与编辑内部组重量/次数，支持添加/删除内部组
- [x] 3.5 扩展数字键盘焦点序列，支持递减组内部组重量/次数输入、上一项/下一项跳转和滚动定位
- [x] 3.6 更新结束训练清理逻辑，未完成递减组和空白内部组不进入完成记录或统计
...
- [x] 4.7 更新训练模板新建与计划详情编辑 UI，支持手动添加、编辑、删除递减组处方及其内部组
- [x] 4.8 保存手动编辑的计划处方到 `setPrescriptions`，并同步维护 `suggested*` 摘要字段
- [x] 4.9 增加计划模板新建/编辑手动递减组处方的单元测试或 UI 回归用例

## 5. iOS 端 / 展示与 Team 摘要

- [x] 5.1 更新训练详情只读流水，递减组显示内部组明细和紧凑摘要
- [x] 5.2 更新训练分享海报，递减组用最大重量内部组作为顶组并表达额外内部组
- [x] 5.3 更新 `CheckinSummary` 结构与渲染，Team 打卡保留和展示递减组内部组
- [x] 5.4 更新 VoiceOver 文案，递减组和内部组输入/详情可被完整朗读
