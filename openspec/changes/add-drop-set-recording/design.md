...
- 「加一组」保留当前普通组行为。
- 「递减组」新增 `setType=drop` 的父级 set，默认 2 个内部组：第 1 组预填上一正式组重量/次数，第 2 组为空；无上一正式组时第 1 组也为空。
- 组级 `⋯` 菜单增加「改为递减组」和「改回普通组」。
- 普通组改递减组：父级重量/次数转为第 1 个内部组，再补第 2 个内部组。
- 递减组改普通组：用第一个有效内部组回填父级，丢弃其它内部组；因会丢数据，需要二次确认。
...
键盘「加一组」在 segment 焦点内保持为新增普通组，不改变现有键名；递减组内部使用行内「添加」按钮添加内部组。
...
计划模板新建与计划详情编辑必须提供手动处方编辑能力：

- 新建训练模板或在计划详情新增动作时，默认仍生成当前的普通处方心智（4 组、10 次、重量为空），但用户可以在动作处方编辑区添加「递减组」。
- 计划处方编辑区复用训练录入的结构心智：普通处方使用重量/次数输入；递减组处方作为 1 个父级处方，内部包含多个有序 segment，每个 segment 编辑重量/次数。
- 计划处方里的递减组没有完成勾选、休息、备注或训练中排序状态；它只保存将来开始训练时要落值的结构。
- 用户在计划模板里添加、删除、转换普通组/递减组后，保存时必须写入 `setPrescriptions`，并同步更新 `suggestedSets/suggestedReps/suggestedWeightKg` 摘要字段用于列表和旧端兼容。
- 若计划项没有手动处方编辑记录，继续按旧 `suggested*` 规则生成普通组。
...
- [Risk] 递减组 UI 增加行高，训练中页面更拥挤。  
  Mitigation：展开态显示完整内部组，折叠/详情摘要显示 `80×8 / 60×6` 或 `80×8 +2组`。
