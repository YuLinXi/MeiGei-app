...
- 增加每个 Team 独立的“Team消息”偏好；关闭后统一停止接收该 Team 的拍一拍、队友打卡和表情回应推送。
- 将“分享动态”和“Team消息”作为 Team 详情页并列设置，新成员默认均为开启，不再提供仅控制拍一拍的独立开关。
...
- `team-ui`: 增加“拍一拍队友”入口、队友状态列表、拍一拍操作与统一 Team 消息偏好交互。
...
- 数据库：增加 Flyway 迁移；`team_member` 增加按 Team 保存且默认开启的统一 Team 消息偏好，并将自动分享默认值调整为开启。
