...
### Requirement: 训练历史动作卡展开与结构展示

已完成训练详情中的普通组、递减组和超级组 SHALL 各自使用可独立展开的动作卡，并且每次进入详情时 MUST 默认展开全部动作明细。点按任一卡片头部 SHALL 只切换该卡片的备注与组/轮明细，MUST NOT 改变其它卡片或训练数据。

所有动作卡头部 SHALL 在左侧展示结构图标（适用时）与单行动作标题，在右侧保留组数与展开状态区域；标题超过可用宽度时 MUST 尾部省略，不得挤压右侧组数。超级组左侧 MUST 显示超级组图标和两个成员动作标题，右侧 MUST 显示其统一轮数对应的 `N 组`，不得合计两个成员的实际 set 数。

#### Scenario: 训练历史默认展开并可独立收起
- **WHEN** 用户打开同时包含普通组、递减组和超级组的已完成训练详情
- **THEN** 三类动作卡都默认展示备注（若有）和全部组/轮明细
- **AND** 用户收起其中一张卡时，仅该卡保留头部，其它卡保持原状态
- **AND** 用户重新打开该训练详情时，全部动作卡再次默认展开

#### Scenario: 超级组头部保留组数区域
- **WHEN** 超级组两个成员动作名称过长
- **THEN** 卡片左侧标题按尾部省略
- **AND** 右侧仍显示超级组轮数形式的 `N 组` 与展开状态
