...
- 在训练进行中页，重排 SHALL 以一级训练单元为粒度更新顺序；普通组、递减组和超级组均作为一个完整单元移动。重排后系统 SHALL 同步更新成员动作的 `WorkoutExercise.orderIndex`、将当前 `Workout` 标脏，并且只影响当前训练记录。
- 训练进行中的超级组存在时，只要训练含两个及以上一级训练单元，排序入口 MUST 保持可见；仅有一个一级训练单元（包括一个超级组）时不得展示该入口。
- 训练进行中的重排 MUST NOT 自动更新来源计划的 `PlanItem.orderIndex`，也 MUST NOT 拆散超级组成员或改变成员执行顺序。
...
#### Scenario: 超级组作为完整训练单元排序
- **WHEN** 训练包含一个普通组与一个超级组，用户在排序面板移动超级组
- **THEN** 排序入口保持可见，超级组整体移动
- **AND** 超级组两个成员保持相邻且执行顺序不变

#### Scenario: 单一超级组不展示排序入口
- **WHEN** 训练只包含一个超级组训练单元
- **THEN** 页面不展示排序入口
