...
- **徽章显示规则**：
  - 热身组：显字母 **`W`**（不参与数字编号）。
  - 正式组：按动作内**仅正式组**的相对顺序显示 **1, 2, 3…**（重新编号，跳过热身组）。
- **「加一组」默认 `working`**：`WorkoutKeypad` 的加一组键追加正式组（预填上一**正式**组重量，热身组重量不作为预填源）。

```
卧推
┌─────────────────────────────┐
│ W   40 × 10   ← 点 W 切回正式 │  热身，不计统计
│ W   60 × 5                   │
│ 1   80 × 8    ← 点 1 切热身   │  正式，从 1 编号
│ 2   80 × 8                   │
│ 3   80 × 7                   │
└─────────────────────────────┘
```
...
| 切换入口 | 序号徽章点击 toggle | 用户选定；两态用 toggle，多态再升 picker |
| 编号 | 仅正式组 1..n，热身显 W | 业界一致的视觉区分 |
