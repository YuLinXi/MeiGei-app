...
- 训练信息以更通透的暖白半透明凭证融合到插画中，至少承载 6 个动作摘要，不遮挡主体，也不沿用旧黑红卡片视觉。
...
凭证保持统一信息顺序：品牌/日期、状态标签、训练标题、核心指标、动作摘要、PR。动作摘要统一至少承载前 6 个动作；超过 6 个时给出“另有 N 个动作”。通过收紧凭证间距、字号和内边距控制高度，避免挤出安全区。底部只保留 `别练了 · DON'T LIFT` 品牌签名。
...
- [训练内容过长] → 凭证至少显示 6 个动作摘要并明确剩余数量，完整数据仍保留在训练详情中；通过紧凑排版而非缩小到不可读解决高度问题。
