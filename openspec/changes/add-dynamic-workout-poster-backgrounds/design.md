...
- 训练信息以更通透的暖白半透明凭证融合到插画中，完整承载全部动作摘要，不遮挡主体，也不沿用旧黑红卡片视觉。
- 背景切换入口与长海报处于同一首屏区域，当前页直接驱动保存与分享。
...
训练凭证使用暖白色 80%～84% 不透明度、细描边和轻阴影；文字使用深暖棕，数字与 PR 使用珊瑚橙。禁止大面积毛玻璃、厚重渐变遮罩和旧黑红卡片结构。
...
凭证保持统一信息顺序：品牌/日期、状态标签、训练标题、核心指标、动作摘要、PR。动作摘要逐条展示全部训练单元，不显示“另有 N 个动作”等省略提示；不超过 6 条时使用单列，超过 6 条时使用双列紧凑布局。通过收紧凭证间距、字号和内边距控制高度，避免挤出安全区。底部只保留 `别练了 · DON'T LIFT` 品牌签名。

### 4. 大海报分页器直接驱动保存与分享

预览首屏使用 `TabView` page style 展示 5 张大海报，支持在海报上左右滑动。海报两侧提供不小于 44pt 的前后切换按钮，海报上方提供 5 张紧凑小预览、当前页标题和 `N / 5` 页码。小预览通过勾选图标和描边共同表达选中态，不只依赖颜色。
...
`WorkoutPosterImageRenderer.render` 接收同一 `background`。滑动、左右按钮和小预览都只更新同一个局部 `@State`；当前页变化后使旧“已保存”状态失效。分页和小预览均不进入导出图片，并提供中文 VoiceOver label/value。切换动画遵守 Reduce Motion。
...
- [训练内容过长] → 动作摘要不做数量截断；超过 6 条后切换双列紧凑布局，以可读的分栏排版承载全部动作，并继续复用固定 9:16 画布。
- [背景主体被遮挡] → 通过 `layoutKind` 使用三种稳定 safe zone，而不是对所有图片套同一位置。
- [推荐和用户意图冲突] → 自动推荐只决定初始值，5 张缩略图始终允许用户覆盖。
- [分页与垂直滚动冲突] → 大海报只处理横向 page gesture，外层 `ScrollView` 继续处理垂直方向；左右按钮提供无手势替代路径。
