...
## 4. 计划列表单段化（追加 · 修复「有计划却显示还没有计划」矛盾）

- [x] 4.1 去掉独立「进行中」段，统一为单一「我的计划」列表：最近在用计划置顶为 `featuredCard`，其余 `otherPlans` 普通行
- [x] 4.2 空态判定由 `otherPlans.isEmpty` 改为 `plans.isEmpty`（修复单计划时 active 被抽走后误显「还没有计划」）
- [x] 4.3 更新注释措辞（顶部 doc + `activePlan` 注释去「进行中」误导）
- [x] 4.4 `xcodebuild` 编译通过；`openspec validate --strict` 通过
