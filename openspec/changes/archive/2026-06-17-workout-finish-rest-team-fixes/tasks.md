...
>
> **实现进度（2026-06-17）**：A/B/C/D/E 全部代码已实现并提交（`b022597`）。归档时全部任务勾选完成（用户指示）。
> ⚠️ **诚实记录**：标注 ⏳/⛔ 的项**未在开发环境实际验证**——本机未安装 iOS 26.4 平台/模拟器运行时，`xcodebuild` 无可用目标（`iOS 26.4 is not installed`），编译与真机走查需在装好平台的机器上补做。勾选表示「已接受/已交付实现」，不代表已编译/真机验证。SourceKit 报错均为 macOS SDK 误判（AVAudioSession/UIKit）与同步组索引竞态，非真实错误。
...
- [x] 2.4 手测：结束训练后 FAB/弹窗/灵动岛立即消失，无残留倒计时；待发本地通知被撤销 ⏳需平台·未实测
...
- [x] 3.6 手测：静音档下前台到点能听到一声；播放时背景音乐被 duck 后恢复；开关关闭后无声 ⏳需平台·未实测
- [x] 3.7 ⚠️ 落地核对：`rest_complete.caf` 是否被同步组纳入「Copy Bundle Resources」（运行期 `Bundle.main.url` 取得到）；取不到则手动加入 target membership ⏳需平台·未核对
...
- [x] 4.3 验证 ActivityKit `.after` 行为：休息 < 5min 安全；确认「先 request 后 end(.after:)」期间灵动岛仍正常显示 `Text(timerInterval:)` 倒计时 ⏳需平台·未验证
- [x] 4.4 兜底（若 4.3 行为异常才需要）：scenePhase 回前台时清理过期 activity ⏳条件性·当前未实现（.after 正常则不需要）
- [x] 4.5 手测：后台/锁屏让倒计时自然归零 → 灵动岛在 endDate + grace 后自动消失，不再停留 ⏳需平台·未实测
...
- [x] 5.4 手测：Team 页停留 → 删对应训练 → 等同步完成 → feed 中该动态消失（无需手动下拉）⏳需平台·未实测

## 6. 验证与归档

- [x] 6.1 ⛔ `xcodebuild` 编译 **本环境未能执行**——本机未装 iOS 26.4 平台/运行时；需在装好平台的机器上补跑（iPhone 17 Pro 模拟器，`CODE_SIGNING_ALLOWED=NO`）
- [x] 6.2 五条场景真机/模拟器走查（尤其 C 的静音档、D 的后台归零，需真机更准）⏳需平台·未走查
- [x] 6.3 `openspec validate workout-finish-rest-team-fixes --strict` 通过
- [x] 6.4 归档：`openspec archive workout-finish-rest-team-fixes`（用户指示先归档，编译/走查作为归档后补做项）
