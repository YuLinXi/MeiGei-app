...
- [x] 3.1 [数据] 整理 150-200 个内置动作（名称/主要肌群/器械类型）
  - [x] 动作清单：`BuiltinExercise.starter` 已扩至 153 个（8 肌群 × 6 器械类型，code 唯一、取值对齐枚举，编译通过）
  - 范围调整（2026-06-10）：部位高亮图资源**移出 MVP 范围**，留待后续单独立项（proposal/spec 已同步去除）
...
- [x] 5.1 端到端联调：登录 → 离线记录 → 同步 → Team 打卡 → 推送 → 表情回应
  - [x] 服务端链路自动化：`scripts/api-e2e.sh` 双用户跑通 登录→同步(push/pull)→建团/加入→打卡 fan-out→表情（真库断言全过）
  - [x] iOS App 内真实交互 + 真实 Apple 登录 / APNs 投递：真机验收通过（2026-06-10）
- [x] 5.2 弱网/离线/多设备冲突场景验证（幂等、last-write-wins、人工提示）
  - [x] 服务端正确性自动化：`scripts/api-e2e.sh` 断言 幂等(serverTime 重放一致)/LWW 冲突回传 serverValue/较新覆盖
  - [x] iOS 端时序与冲突提示 UI：手动测试通过（2026-06-10）
