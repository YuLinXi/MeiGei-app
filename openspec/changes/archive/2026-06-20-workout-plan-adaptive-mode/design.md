...
**问题**：自适应模式的开始训练落值并不总等于 `PlanItem.suggested*`。当存在历史完成组时，`PlanPrefill` 会优先使用上次同动作 completed 正式组；当计划项缺组数且无历史时，又会 fallback 到 4 组。这意味着计划详情如果只展示 `suggestedSets/Reps/WeightKg`，用户看到的可能不是点「开始这次训练」后真正会预填的内容。

**决定**：计划详情从「模板字段展示」升级为「下次训练处方」：

```
计划项 + 训练历史 ──PlanPrefill──▶ 下次有效处方 ──展示──▶ 用户
                                 └─开始训练──▶ WorkoutSet 落值
```

动作行 SHALL 展示：

- **主摘要**：`下次 N 组 · W kg × R` / `下次 N 组 × R` / `下次 N 组`，来自实际将生成的预填组。
- **来源标签**：`历史` / `预设` / `默认` / `保留` / `严格`，用于扫读。
- **来源说明**：如「来自上次完成 · 昨天」「计划预设」「默认起步」「上次未练 · 已保留」「严格执行 · 完成后不更新」。

来源判定：

| 模式 | 条件 | 标签 | 说明 |
|------|------|------|------|
| strict | 任意严格计划项 | 严格 | 严格执行 · 完成后不更新 |
| adaptive | 命中上次 completed 正式组 | 历史 | 来自上次完成 · 相对时间 |
| adaptive | 未命中历史且有计划建议值 | 预设 | 计划预设 |
| adaptive | 未命中历史且缺少计划建议值 | 默认 | 默认起步 |
| adaptive | 计划项存在但最近一次该计划训练未完成此动作 | 保留 | 上次未练 · 已保留 |

实现上应把处方预览做成纯逻辑 helper，复用或包裹 `PlanPrefill.sets`，避免页面摘要和开始训练落值规则漂移。列表页普通卡同步展示模式与短行为摘要，避免用户点进详情才知道计划是否会自动回写。
...
- 交互语义：计划详情右上角更多菜单不承载模式入口，避免把高影响行为混在重命名/删除等模板操作里。正文模式说明卡仍可打开模式 sheet。sheet 内点选仅改变本地草稿；点击「确定」后先校验严格模式必填，再展示二次确认；用户确认后才写入 `WorkoutPlan.mode`、`markDirty` 并保存。
