...
# 作用：装 Docker → 建共享网络 → 生成机密 → 起共享 PostgreSQL + MeiGei app → 本地验证 health。
# 不做：不开 80/443、不起 Caddy（等 ICP 备案通过后再 enable-https）。
# 幂等：重复执行安全；已生成的 .env 会复用（不会重置数据库密码）。
set -euo pipefail

REPO_DIR="${REPO_DIR:-/opt/MeiGei-app/backend}"
...
elif [ -f "$DB_ENV" ]  && grep -q '^MEIGEI_DB_PASSWORD='  "$DB_ENV";  then DB_PW="$(grep '^MEIGEI_DB_PASSWORD='  "$DB_ENV"  | cut -d= -f2-)"
...
MEIGEI_DB_PASSWORD=$DB_PW
EOF
else log "复用已有 $DB_ENV"; fi

if [ ! -f "$APP_ENV" ]; then
  log "生成 MeiGei 业务机密 $APP_ENV"
...
# ── 5. 起共享 PostgreSQL ───────────────────────────────
log "启动共享 PostgreSQL（首次会自动建 meigei 库 + 角色）"
( cd "$STACKS_DIR/db" && $DC compose up -d )

# ── 6. 起 MeiGei app（首次编译镜像较久）────────────────
log "构建并启动 MeiGei app（首次 2-5 分钟）"
...
for i in $(seq 1 60); do
  if $DC run --rm --network web busybox:latest \
       wget -qO- http://meigei-app:8080/actuator/health 2>/dev/null | grep -q '"status":"UP"'; then
    log "✅ 后端就绪：/actuator/health = UP"
    log "下一步（备案通过后）：配 DNS meigei.peipadada.com → 本机IP，然后 cd $STACKS_DIR/edge && docker compose up -d 起 Caddy 签 HTTPS"
    exit 0
  fi
  sleep 5
done
